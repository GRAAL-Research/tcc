...
def _get_num_words(sp_object: Union[Doc, List[Token]], min_size: int = 0, get_size: bool = False, get_unique: bool = False,
                   without_stop: bool = False) -> Union[int, Tuple[int, int]]:
    ...
    """
    filtered_words = [
        word.text.lower() for word in sp_object if not word.is_punct and "'" not in word.text and len(word.text) >= min_size
