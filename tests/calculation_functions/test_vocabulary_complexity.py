...
from TextComplexityComputer.calculation_functions import vocabulary_complexity as vc
...
class TestPa(TestCase):
    def test_givenText_thenGetMetric(self):
        ...
        self.assertEqual(vc.pa(sp_object), 0.25)
...
class TestUniGramLem(TestCase):
    def test_givenText_thenGetMetric(self):
        # see vocbulary_complexity.unigram_lem for calculation
        text = 'il mange une chaussure'
        sp_object = tcc.get_sp_object(text)
        self.assertEqual(vc.uni_gram_lem(sp_object), -8.32673949170022)
    ...
    def test_givenTextWithZeroProbabilityWord_thenGetMetric(self):
        ...
        sp_object = tcc.get_sp_object(text)
        self.assertEqual(vc.uni_gram_lem(sp_object), -9.454065176893348)
    ...
    def test_givenTextWithUnknownWord_thenGetMetric(self):
        ...
        sp_object = tcc.get_sp_object(text)
        self.assertEqual(vc.uni_gram_lem(sp_object), -7.500609988400598)
