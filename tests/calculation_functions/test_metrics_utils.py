...
from TextComplexityComputer.calculation_functions import metrics_utils as mu
...
class TestGetNumSentences(TestCase):
    ...
    def test_givenTextWithApposition_thenGetNumSent(self):
        text = 'Il joue au football qui est le sport: des faibles; des victimes; des stars; des forts; des fans;' \
               ' et des malhonnêtes.'
        sp_object = tcc.get_sp_object(text)
        self.assertEqual(mu._get_num_sentences(sp_object), 1)

    def test_givenTextWithPunctuationsAndQuotes_thenGetNumSent(self):
        text = 'Il joue au football ! C\'est qui est le sport: des faibles; des victimes; et des malhonnêtes. ' \
               'Quel tristesse?'
        sp_object = tcc.get_sp_object(text)
...
class TestGetNumSyllables(TestCase):
    ...
    def test_givenTextWithAllAccents_thenGetNumSyllabe(self):
        ...
        text = 'Portez ce vieux whisky au juge blond qui fume sur son île intérieure, à côté de l\'alcôve ovoïde, où ' \
